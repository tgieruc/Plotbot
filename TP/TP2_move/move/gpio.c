...
void gpio_config_output_pushpull_floating(GPIO_TypeDef *port, unsigned int pin)
{
    // Output type pushpull : OTy = 0
    port->OTYPER &= ~(1 << pin);
...
void gpio_config_output_open_drain_floating(GPIO_TypeDef *port, unsigned int pin)
{
    // Output type open-drain : OTy = 1
    port->OTYPER |= (1 << pin);
...
void gpio_config_output_af_pushpull(GPIO_TypeDef *port, unsigned int pin, unsigned int afrh){
	// Output type pushpull : OTy = 0
	port->OTYPER &= ~(1 << pin);
	...
	// Alternative mode : MODERy = 10
	port->MODER = (port->MODER & ~(1 << (pin * 2))) | (2 << (pin * 2));

	// set to AFR2
	int half = pin / 8;

	port->AFR[half] = (port->AFR[half] & ~(15 << ((pin * 4)%32))) | (afrh << ((pin * 4)%32));
}
